...
- [Run your own instance of Simulated Hospital](#run-your-own-instance-of-simulated-hospital)
  - [Download](#download)
  - [Run with Go (recommended)](#run-with-go-recommended)
  - [Run with Bazel (legacy)](#run-with-bazel-legacy)
  - [Create Docker image (legacy)](#create-docker-image-legacy)
  - [Publish Docker image (legacy)](#publish-docker-image-legacy)
...
> **Deprecated:** The original public image `eu.gcr.io/simhospital-images/simhospital:latest`
> is no longer actively maintained/published. Prefer running from source with
> the Go toolchain.
...
Prerequisites: install [Go](https://go.dev/dl/) (1.22+) and
...
./simulator --local_path=${LOCAL_DIR}
```

Stop the simulator with Ctrl-C.

You need to set the `local_path` argument when running locally so that
Simulated Hospital knows where to find the default configuration files.
Alternatively, you can set the value of all of the
[data configuration arguments](./arguments.md#data-configuration).
...
See [run-with-go.md](./run-with-go.md) for the full guide, including dashboard
and MLLP output options.

### Run with Bazel (legacy)

> **Legacy path:** This workflow is kept for reference. It is not reliable on
> modern Bazel versions and Apple Silicon environments.
...
### Create Docker image (legacy)

> **Legacy path:** This flow depends on the Bazel setup above.
...
1.  Build the image. You will see the name of the repository and tag
    _(bazel:simhospital_container_image)_ printed at the end:
...
1.  Get a Simulated Hospital Docker image. You can either
    [create a Docker image from your local code](#create-docker-image-legacy), or
    download an image from a registry (see
    [Publish Docker image](#publish-docker-image-legacy) for how to push and pull your
...
know where to find the configuration files. When running locally (Go or Bazel),
set the `-local_path` argument to tell Simulated Hospital where to find your
files.
